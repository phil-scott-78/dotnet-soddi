...
/// <summary>
/// SQL Server bulk data insertion implementation
/// </summary>
[UsedImplicitly]
public class SqlServerDataInserter : IDataInserter
{
    public SqlServerDataInserter(IFileSystem fileSystem)
    {
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var batchSize = tableName.ToLowerInvariant() switch
        {
            "posts" => 500,
            "posthistory" => 500,
            "comments" => 2000,
        ...
        var retryPolicy = Policy
            ...
            .WaitAndRetryForeverAsync(_ => TimeSpan.FromMilliseconds(500), (ex, _, _) =>
                {
                    Log.Write(LogLevel.Trace, $"Exception occurred, but we'll retry. {ex.Message}");
        ...
        await retryPolicy.ExecuteAsync(async () =>
        {
            ...
            if (totalCopiedSoFar > 0)
            {
                ...
                // written. most of it should be committed to the database, but we are going to be in a state where we know
                // quite what was written. We've saved the last few batches of data in the buffer for replaying, but
                ...
                var keys = tableName.ToLowerInvariant() switch
                {
                    "posttags" => new[] { "postid", "tag" },
                    _ => new[] { "id" }
                };
            ...
            bc.SqlRowsCopied += (_, args) =>
            {
                ...
                progress.Report(totalCopiedSoFar);
            };
